...
func TestNewForState_PrePopulates(t *testing.T) {
	...
	state := runtimestate.NewOrchestrationRuntimeState("inst", nil, []*protos.HistoryEvent{
...
func BenchmarkLoadHistory(b *testing.B) {
	for _, n := range []int{100, 1000, 5000} {
		...
		b.Run("N="+strconv.Itoa(n), func(b *testing.B) {
			b.ReportAllocs()
			for i := 0; i < b.N; i++ {
				_ = runtimestate.NewOrchestrationRuntimeState("inst", wrapperspb.String(""), history)
			}
		})
	}
}
